...
import { Category, Platform, SortBy } from "@/types"
import { cn } from "@/utils"
import { useI18n } from "@/i18n/LanguageProvider"

const PLATFORMS: Platform[] = ["youtube", "tiktok", "instagram", "twitter", "facebook"]
const CATEGORIES: Category[] = ["All Categories", "Music", "Gaming", "Entertainment", "Sports", "Lifestyle", "Meme"]
const SORT_OPTIONS: SortBy[] = ["Featured", "Newest", "Highest Budget", "Highest Available Budget", "Highest CPM", "Most Paid Out", "Most Creators"]
...
export function FilterBar() {
  ...
  return (
    <div ref={ref} className="flex items-center gap-2 flex-wrap relative">
      ...
      <button onClick={() => setOpen(open === "sort" ? null : "sort")} className="px-3 py-2 bg-white border border-border rounded-xl">
      ...
      {open === "sort" && (
        <div className="absolute top-full mt-1 left-0 z-50 bg-white border rounded-xl shadow-lg py-1 min-w-56">
          {SORT_OPTIONS.map((s) => (
            <button key={s} onClick={() => { setFilter("sortBy", s); setOpen(null) }} className="w-full text-left px-4 py-2 text-sm hover:bg-secondary">
              {t(`sort.${s}`)}
            </button>
          ))}
        </div>
